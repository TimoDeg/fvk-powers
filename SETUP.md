...
> Ich richte fvk-powers für dich ein. Dafür prüfe ich den Zugriff auf Jira und auf die Rewrite-Specs. Ich schaue zuerst, was schon vorhanden ist, und führe dich dann durch die fehlenden Schritte. Für diesen Quellencheck muss die Produktanwendung nicht laufen.

Verwende Deutsch und das Entwicklerprofil als Standard; das PM-Ausgabeprofil gilt bei entsprechendem Wunsch. Frage nicht zuerst nach Rolle, Modell, technischen Einstellungen oder Antwortlänge. Setze weder Codex noch ein ChatGPT-Konto voraus; derselbe Ablauf gilt für alle Clients mit den benötigten Fähigkeiten. Explizite Wünsche des Nutzers übernehmen.
...
Die [Voraussetzungen](docs/DEPENDENCIES.md) unterscheiden nötige Zugänge von optionalen Werkzeugen. Der [Kontexteinstieg](CONTEXT.md) beschreibt weitere Originalquellen. Lade daraus nur das gerade Benötigte. Die optionale Python-Paketprüfung ist keine Voraussetzung für den Quellenzugriff und ersetzt keinen Jira-Test.

Fehlt eine Grundvoraussetzung, verwende die [Installationsschritte](docs/DEPENDENCIES.md#einrichtung-von-anfang-an) und erkläre genau den nächsten passenden Schritt im Chat. Ermittle Betriebssystem und Client aus verfügbaren Angaben; frage nur, wenn die nächste Anleitung davon abhängt. Prüfe, ob `AGENTS.md` und das gewählte Profil (standardmäßig `profiles/developer.md`) im aktuellen Projekt lesbar sind, und beim lokalen Git-Weg mit `git --version`, ob Git verfügbar ist. Vorhandene Installationen wiederverwenden; bei fehlendem Git nur den zum Betriebssystem passenden Installationsweg anbieten. Für diesen Quellencheck keine Produkt-Buildwerkzeuge verlangen. Vor einer beauftragten Implementierung oder Testausführung die nötige Produktumgebung anhand der Repo-Anleitung prüfen; ein erfolgreicher Quellencheck bestätigt noch keine lauffähige Entwicklungsumgebung. Die Installation des gewählten Assistenten selbst liegt vor diesem Projektchat; Nutzer, die dabei noch festhängen, anhand der Anleitung weiterführen.
...
- Fehlt der Connector, ermittle zuerst den tatsächlich verwendeten Client. Bei einem Multiplexer wie Orca ist der darin gestartete Agent maßgeblich. Ist er aus dem Kontext nicht erkennbar, frage nur danach. Wähle dann den passenden Weg aus [Jira verbinden](docs/DEPENDENCIES.md#jira-verbinden): je nach Client Plugin, Connector oder MCP-Verbindung. Erkläre den nächsten nötigen Schritt direkt im Chat. Codex-Menüs oder `/plugins` nicht auf Cursor, Claude oder Orca übertragen. Bei unbekanntem Client oder abweichenden Menüs dessen offizielle Anleitung bzw. tatsächlich angebotene Optionen prüfen, statt Schaltflächen zu raten. Lade die Werkzeuge nach Einrichtung so neu, wie der Client es verlangt; falls ein neuer Chat nötig ist, dort mit „Setup weiter“ und gespeicherten Angaben fortsetzen. Anschließend das konkrete Ticket tatsächlich lesen. Installiere nichts ungefragt und bitte nie um Tokens, Passwörter oder Sitzungscookies im Chat. Fehlende Connector-Verfügbarkeit oder Workspace-Freigabe an den Administrator verweisen, fehlende Ticketrechte davon getrennt benennen.
...
> Jira und Rewrite-Specs sind erreichbar. Das genannte Ticket habe ich mit der passenden Spec abgeglichen und die Quellen lokal eingerichtet. Du kannst jetzt zum Beispiel fragen: „Welche Codepfade sind betroffen und welche Tests brauche ich?“
